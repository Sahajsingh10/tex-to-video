...
def add_narration_to_video(narrations, input_video, output_dir, output_file):
    ...
    for i, _ in enumerate(narrations):
        # Load the narration audio file
        audio = os.path.join(output_dir, "narrations", f"narration_{i+1}.mp3")

        narration_segment = AudioSegment.from_file(audio)
        
        
        # Add the adjusted segment to the full narration
        full_narration += narration_segment

    # Export the combined narration
    ...
    # Remove the temporary narration file
...
def create(narrations, output_dir, output_filename, caption_settings: dict | None = None):
    ...
    # Define the dimensions and frame rate of the video for horizontal layout
    width, height = 1920, 1080  # Horizontal video dimensions
    ...
    for i in range(image_count):
        ...
        for _ in range(math.floor(duration / 1000 * 30)):
            horizontal_video_frame = np.zeros((height, width, 3), dtype=np.uint8)
            x_offset = (width - image1.shape[1]) // 2  # Center the image horizontally
            horizontal_video_frame[:, x_offset:x_offset + image1.shape[1]] = image1

            out.write(horizontal_video_frame)

        for alpha in np.linspace(0, 1, math.floor(fade_time / 1000 * 30)):
            blended_image = cv2.addWeighted(image1, 1 - alpha, image2, alpha, 0)
            horizontal_video_frame = np.zeros((height, width, 3), dtype=np.uint8)
            x_offset = (width - blended_image.shape[1]) // 2  # Center the blended image horizontally
            horizontal_video_frame[:, x_offset:x_offset + blended_image.shape[1]] = blended_image
